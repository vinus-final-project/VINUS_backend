...
CATEGORY_KEYWORDS = {
    ...
    "커피": "커피/라떼", "커피 종류": "커피/라떼", "커피 메뉴": "커피/라떼",
    "라떼": "커피/라떼", "라떼 종류": "커피/라떼", "라떼 메뉴": "커피/라떼",
    "음료": "음료/에이드/스무디", "에이드": "음료/에이드/스무디", "스무디": "음료/에이드/스무디",
    "차": "차", "차 메뉴": "차", "차 종류": "차", "차 보여": "차", "차 류": "차",
    "티": "차", "티 메뉴": "차", "티 종류": "차", "티 보여": "차", "티 류": "차",
    "요거트": "요거트",
...
# 옵션 그룹 질의 ("시럽 뭐 있어?" / "옵션 뭐 있어?") — 현재 메뉴의
# 옵션 안내. 값 = 메뉴 스냅샷 og_name 부분일치용 표준 그룹 단어.
# 리스트 키워드(OPTION_LIST_QUERY_KEYWORDS) 동반 시에만 발동 —
# "얼음" 단독 발화가 옵션 질의로 오인되지 않게.
OPTION_QUERY_GROUPS = {
    "시럽": "시럽", "샷": "샷", "휘핑": "휘핑", "온도": "온도",
    "사이즈": "사이즈", "크기": "사이즈", "얼음": "얼음",
    "당도": "당도", "펄": "펄",
}
OPTION_LIST_QUERY_KEYWORDS = ("뭐 있", "뭐가 있", "종류", "뭐 돼", "뭐 되")
...
# 메뉴 낭독 발화에서 제거할 일반어 — 이걸 다 빼고도 단어가 남으면
# 재료/속성 검색("딸기 뭐 있어")으로 보고 RECOMMEND 로 라우팅한다.
MENU_LIST_RESIDUAL_STOPWORDS = (
    "메뉴", "종류", "전체", "모든", "무슨", "어떤",
    "들어간", "들어가", "들어있", "있는", "파는", "나오는",
    "거", "것", "좀", "줘", "해줘", "알려", "읽어", "보여", "말해",
)

# --- 알레르기 선언 (음성 캡처) ---
# 선언 마커: 이 표현들이 있으면 "알레르기 선언"으로 판단
ALLERGY_DECLARE_KEYWORDS = ("알레르기", "알러지", "못 먹", "못먹", "먹으면 안", "알러지가", "알레르기가")
# 발화 별칭 → 식약처 표준 알레르기명 (문서 매칭용). 긴 별칭 우선 매칭.
ALLERGEN_ALIASES = {
    "유제품": "우유", "우유": "우유",
    "달걀": "난류", "계란": "난류", "난류": "난류",
    "밀가루": "밀", "글루텐": "밀", "밀": "밀",
...
# 원재료/성분/알레르기 "문의" — 안전상 AI가 답하지 않고 직원 안내로 넘긴다.
#   (가격/설명/"얼마" 는 INFO 그대로 답변 유지)
INGREDIENT_INFO_KEYWORDS = ("들어가", "성분", "원재료", "재료", "알레르기", "알러지")
# 추천 시 일회성 제외 표현 ("우유 없는/빼고 추천") — 알레르겐과 함께 오면 exclude
RECOMMEND_EXCLUDE_KEYWORDS = ("없는", "없이", "안 들어간", "안들어간", "안 들어가", "빼고", "빼", "말고", "제외")
